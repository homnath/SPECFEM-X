...
integer :: nelmt_interface,nnode_interface
integer,dimension(:),allocatable :: elmt_interface,part_interface
integer,dimension(:),allocatable :: node_npart,gnode_npart
...
type(gnodes),dimension(:),allocatable :: gnode
...
integer,dimension(0:NGNOD*nelmt-1),intent(in) :: g_num
...
do i = 0, NGNOD*nelmt-1
    nodeselmt(g_num(i)*nsize+nnodeelmt(g_num(i))) = i/NGNOD
...
do j = 0, nnode-1
    do k = 0, nnodeelmt(j)-1
      do l = k+1, nnodeelmt(j)-1
          ...
          do n = 1, NGNOD
            num_node = g_num(NGNOD*elem_base+n-1)
...
integer,dimension(:),allocatable :: ielmts
...
print*,trim(inp_path)//trim(bcfile)
...
bc: do ! ibc=1,nbc
  ...
  elseif(bctype==200)then ! infinite BC
  ...
  allocate(ielmts(bc_nelmt))
  ielmts=bc_elmt(1,:)
  bc_elmt(1,:)=glob2loc_elmt(ielmts) ! local element numbering in the partition
  deallocate(ielmts)
  ...
  do i_part=1,npart
    ! open output file
    write(out_fname, fmt=format_str)trim(out_path)//trim(bcfile)//'_proc',     &
    i_part-1
    if(ibc==1)then
...
subroutine write_ssbcFS(out_path,inp_path,fsfile,part,npart,glob2loc_elmt)
! this subroutine writes the partitioned boundary conditions
implicit none
integer,dimension(:),intent(in) :: part ! numbering starts from 1 only in this routine
integer,intent(in) :: npart
integer,dimension(:),intent(in):: glob2loc_elmt ! numbering starts from 1 only in this routine
character(len=250),intent(in) :: inp_path,out_path
character(len=80),intent(in) :: fsfile
character(len=20) :: format_str,format_str1
character(len=80) :: out_fname
...
! open bc file
open(unit=16,file=trim(inp_path)//trim(fsfile),status='old',action='read',iostat=istat)
if(istat/=0) then
  print*,'error file open:',trim(fsfile)
  stop
endif
print*,trim(inp_path)//trim(fsfile)
! format string for file name
write(format_str,*)ceiling(log10(real(npart)+1.))
format_str='(a,i'//trim(adjustl(format_str))//'.'//trim(adjustl(format_str))//')'

read(16,*,iostat=ios)bc_nelmt
if(ios/=0.or.bc_nelmt.eq.0)then
  ! create empty BC files for all partitions
  do i_part=1,npart
    ! open output file
    write(out_fname, fmt=format_str)trim(out_path)//trim(fsfile)//'_proc',i_part-1
    open(unit=17,file=trim(out_fname),status='replace',action='write',iostat=istat)
    write(17,'(i0)')0
    close(17)
  enddo ! i_part
...
do i_part=1,npart
  ! open output file
  write(out_fname, fmt=format_str)trim(out_path)//trim(fsfile)//'_proc',     &
  i_part-1
  open(unit=17,file=trim(out_fname),status='replace',action='write',       &
  iostat=istat)
  if(istat/=0) then
...
end subroutine write_ssbcFS
!===============================================================================

subroutine write_ssbcINF(out_path,inp_path,bcfile,part,npart,glob2loc_elmt)
! this subroutine writes the partitioned boundary conditions
implicit none
integer,dimension(:),intent(in) :: part ! numbering starts from 1 only in this routine
integer,intent(in) :: npart
...
bc: do ! ibc=1,nbc
  ...
  ! format string for element ID and face ID
  write(format_str1,*)ceiling(log10(real(maxval(bc_elmt(1,:)))+1.))
  ! i2 is sufficient for face id or inf dir
  format_str1='(i'//trim(adjustl(format_str1))//',1x,i1,1x,i1)'
...
! prepare magnet switch
ismagnet=.false.
if(present(ismagnetpar))then
  if(ismagnetpar)ismagnet=.true.
endif
! open traction file
...
if(tractype==0)then ! point loading
  if(.not.ismagnet)read(16,*)q0
elseif(tractype==1)then ! uniform loading
  if(ismagnet)then
    read(11,*)M0,incORlat,rval,dec
  else
    read(16,*)q0
  endif
...
format_str1='(i'//trim(adjustl(format_str1))//',1x,i2)'
...
subroutine write_faultslip(out_path,inp_path,fsfile,part,npart,glob2loc_elmt)
! this subroutine writes the partitioned boundary conditions
use global,only:g_num
...
character(len=60) :: format_str,format_str1
character(len=80) :: out_fname
...
integer,dimension(:),allocatable :: ielmts
integer :: ios,i_edge,i_elmt,i_part,ifs,ipart,istat,fstype

integer :: ielmt,iface

! fault slip vector
real(kind=kreal) :: fsvec(3) ! replace this with NDIM
logical :: ispart(npart)
integer,allocatable :: temp_mat(:,:)
type master_partition
  integer,dimension(:),allocatable :: iloc
...
open(unit=16,file=trim(newfsfile),status='old',     &
action='read',iostat=istat)
if(istat/=0) then
  print*,'error file open:',trim(newfsfile)
  stop
endif
...
format_str='(a,i'//trim(adjustl(format_str))//'.'//trim(adjustl(format_str))//')'
ifs=0
fsurface: do ! ifs=1,nfs
  read(16,*,iostat=ios)fsvec
  if(ios/=0)then
    if(ifs.eq.0)then
      ! create empty faultslip files for all partitions
      do i_part=1,npart
        ! open output file
        write(out_fname, fmt=format_str)trim(out_path)//trim(fsfile)//&
        '_recreated'//'_proc', &
        i_part-1
        open(unit=17,file=trim(out_fname),status='replace',action='write',     &
        iostat=istat)
  ...
  ifs=ifs+1
  !if(fstype==0)then ! point
  !  write(*,*)'ERROR: point slip fault is not unsupported!'
  !  stop
  !elseif(fstype==1)then ! edge
  !  write(*,*)'ERROR: edge slip fault is not unsupported!'
  !  stop
  !elseif(fstype==2)then ! face

  !elseif(fstype==21)then ! fault

  !else
  !  write(*,*)'ERROR: unsupported fault_slip type:',fstype
  !  stop
  !endif
  read(16,*)fs_nelmt
  if(fs_nelmt>0)then
    allocate(fs_elmt(6,fs_nelmt))
  ...
  do i_elmt=1,fs_nelmt
    mpart_nelmt(part(fs_elmt(1,i_elmt)))=mpart_nelmt(part(fs_elmt(1,i_elmt)))+1
  enddo
  ! allocate derived type variables
  ispart=.false.
  do i_part=1,npart
  ...
  ! partition BC
  do i_elmt=1,fs_nelmt
    ipart=part(fs_elmt(1,i_elmt)) ! partion
    mpart_icount(ipart)=mpart_icount(ipart)+1
    mpart(ipart)%iloc(mpart_icount(ipart))=i_elmt
  enddo
  ...
  ! NOTE: surprisingly intel Fortran 19 compiler gives the strange error 
  ! for a following statement.
  !fs_elmt(1,:)=glob2loc_elmt(fs_elmt(1,:))
  allocate(ielmts(fs_nelmt))
  ielmts=fs_elmt(1,:)
  fs_elmt(1,:)=glob2loc_elmt(ielmts)
  deallocate(ielmts)
  ...
  do i_part=1,npart
    ! open output file
    write(out_fname, fmt=format_str)trim(out_path)//trim(fsfile)//'_recreated'//'_proc',     &
    i_part-1
    if(ifs==1)then
      open(unit=17,file=trim(out_fname),status='replace',action='write',       &
      iostat=istat)
    ...
    if(ispart(i_part))then
      ...
      elseif(fstype==6)then ! fault

      endif
      write(17,*)mpart_nelmt(i_part)
      allocate(temp_mat(6,mpart_nelmt(i_part)))
      temp_mat=fs_elmt(:,mpart(i_part)%iloc)
      write(17,format_str1)temp_mat
      deallocate(temp_mat)
    endif
    close(17)
  enddo ! i_part
...
write(*,'(a,i3,a)',advance='no')'nfs=',ifs,'...'

end subroutine write_faultslip
!===============================================================================
...
!integer,dimension(NGNOD,nelmt),intent(in) :: connect
! numbering starts from 1 only in this routine
integer,dimension(:),intent(in) :: part
...
integer :: i_elmt,i_node,n1,n2
integer :: inode,inode0
integer :: ngnode_all
...
! connectivity of the interface elements
allocate(connect_interface(NGNOD,nelmt_interface))
!connect_interface=connect(:,elmt_interface)

! store interace connectivity in a compressed format
ngnode_all=NGNOD*nelmt_interface
allocate(nodelist(ngnode_all),inode_order(ngnode_all))

n1=1; n2=NGNOD
do i_elmt=1,nelmt_interface
  
  ! Address for new connectivity
  connect_interface(:,i_elmt)= (/ (i, i=n1,n2) /)
  nodelist(n1:n2)=connect(:,elmt_interface(i_elmt))
 
  ! Do NOT modify these parameters after the line below
  n1=n2+1; n2=n1+NGNOD-1
...
nnode_interface=maxval(inode_order)
allocate(gnode_npart(nnode_interface),isnode(nnode_interface))
isnode=.false.
allocate(gnode(nnode_interface))
do i_node=1,ngnode_all
  inode=inode_order(i_node)
  if(.not.isnode(inode))then
    inode0=nodelist(i_node)
    gnode_npart(inode)=node_npart(inode0)
    allocate(gnode(inode)%part(gnode_npart(inode)))
    gnode(inode)%part=node(inode0)%part
    isnode(inode)=.true.
  endif
enddo
! new connectivity
do i_elmt=1,nelmt_interface
  connect_interface(:,i_elmt)=inode_order(connect_interface(:,i_elmt))
enddo
deallocate(nodelist,inode_order)
...
!adj_elmt=0
allocate(indadj(nnode_interface))
...
do i=1,nelmt_interface
  ...
  do j=1,nenode
    ...
    do k=1,gnode_npart(inode)! node_npart(inode)
      !mpart_gpart(part_interface(i),node(inode)%part(k))=.true.
      mpart_gpart(part_interface(i),gnode(inode)%part(k))=.true.
    enddo
  enddo
enddo
do i_node=1,nnode_interface
  deallocate(gnode(i_node)%part)
enddo
deallocate(gnode,gnode_npart,connect_interface)

do i=1,nnode
  deallocate(node(i)%part)
enddo
deallocate(node,node_npart)
...
!===============================================================================
  ...
  !--------------------------------------------------
   subroutine build_interfaces(nelmt,sup_neighbor,part,g_num,xadj,adjncy, &
    ...
    integer, dimension(0:sup_neighbor*nelmt-1), intent(in)  :: adjncy
                   ...
                   if (part(adjncy(el_adj)) == num_part_bis) then
                      ...
                      do num_node = 0, NGNOD-1
                         do num_node_bis = 0, NGNOD-1
                            if (g_num(el*NGNOD+num_node) == g_num(adjncy(el_adj)*NGNOD+num_node_bis)) then
                               tab_interfaces(tab_size_interfaces(num_interface)*7+num_edge*7+3+ncommon_nodes) &
                                    = g_num(el*NGNOD+num_node)
...
nvisco,imat_visco,visco_rheology,nmaxwell,muratio,viscosity, &
nwmat,waterid,iproc,npart)
...
integer,intent(in) :: nmaxwell
character(len=20),intent(in) :: visco_rheology
real(kind=kreal) :: muratio(nmaxwell,nvisco),viscosity(nvisco)
...
if(nvisco.gt.0)write(16,*)trim(visco_rheology)
do i=1,nvisco
...
integer,dimension(0:NGNOD*nelmt-1) :: g_num
...
subroutine write_interfaces_database(out_phead,tab_interfaces,                 &
                                     tab_size_interfaces,iproc,ninterfaces,    &
                                     my_ninterface,my_interfaces,              &
                                     my_nb_interfaces,glob2loc_elmnts,         &
...
integer :: istat
character(len=60),parameter :: sline1='myrank (1)',                            &
                               sline2='number of ghost partitions (1)',        &
                               sline3='ghost rank (1)',                        &
                               sline4='ghost nelmt (1)',                       &
                               sline5='element ID (1), entity type (1), entity node IDs (4)'
character(len=20) :: format_str
character(len=80) :: out_fname
...
! counts number of interfaces to neighboring partitions
   ...
   do i = 0, nparts-1
      do j = i+1, nparts-1
         ! only counts if specified partition (iproc) appears and 
         ! interface elements increment
  ...
  ! file name for MPI interfaces
  ! following two lines can be placed outside the loop and passed as an
  ! argument
  write(format_str,*)ceiling(log10(real(nparts)+1.))
  format_str='(a,i'//trim(adjustl(format_str))//'.'//trim(adjustl(format_str))//')'
  write(out_fname, fmt=format_str)trim(out_phead)//'ghost_proc',iproc
  open(unit=16,file=trim(out_fname),access='stream',form='unformatted', &
  status='replace',action='write',iostat=istat)
  if (istat /= 0)then
    write(*,'(a)')'ERROR: file "'//trim(out_fname)//'" cannot be opened!'
    stop
  endif
  write(16)sline1
  write(16)iproc
  write(16)sline2
  write(16)my_ninterface
  ...
  do i = 0, nparts-1
     do j = i+1, nparts-1
        if (my_interfaces(num_interface) == 1) then
           ...
           write(16)sline4
           write(16)my_nb_interfaces(num_interface)
           write(16)sline5
           ...
           do k = tab_size_interfaces(num_interface),tab_size_interfaces(num_interface+1)-1
              ...
              case default
                 print *,"ERROR: write_interfaces_database:", tab_interfaces(k*7+2), iproc
                 stop
              end select
           enddo
...
print*,'num_material:',minval(num_material),maxval(num_material)
...
!--------------------------------------------------
! Repartitioning : TWO coupled acoustic/elastic elements are transfered to
! the same partition
!--------------------------------------------------
