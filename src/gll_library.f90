...
! X
real(kind=kreal),allocatable :: gllpx(:),gllwx(:)
! Y
real(kind=kreal),allocatable :: gllpy(:),gllwy(:)
! Z
real(kind=kreal),allocatable :: gllpz(:),gllwz(:)
...
if(mod(ngllx,2) /= 0)gllpx((ngllx-1)/2+1) = zero
! Y
allocate(gllpy(nglly),gllwy(nglly))
call zwgljd(gllpy,gllwy,nglly,jacobi_alpha,jacobi_beta)
if(mod(nglly,2) /= 0)gllpy((nglly-1)/2+1) = zero
! Z
...
! This subroutine cleanup gll1d
subroutine cleanup_gll1d()
implicit none

! X
deallocate(gllpx,gllwx)
! Y
deallocate(gllpy,gllwy)
! Z
deallocate(gllpz,gllwz)

end subroutine cleanup_gll1d
!===============================================================================
...
real(kind=kreal),dimension(ndim,ngll) :: gll_points
...
implicit none
integer,intent(in) :: ndim,ngllx,nglly,ngllz,ngll
real(kind=kreal),intent(in) :: xi,eta,zeta
real(kind=kreal),dimension(ngll),intent(out) :: lagrange_gll
real(kind=kreal),dimension(ndim,ngll),intent(out) :: dlagrange_gll
...
dx=2.0_kreal/real((nenode-1),kreal)! length = 2.0 as xi is taken -1 to +1
do i=1,nenode
...
real(kind=kreal),dimension(nenod) :: xii
real(kind=kreal),dimension(nenod-1) :: term,dterm,sum_term
real(kind=kreal) :: dx
...
! Check ubound
if( ubound(phi,1).ne.nenod .or. &
    ubound(dphi_dxi,1).ne.nenod)then
...
implicit none
integer,intent(in) :: nenod ! number of nodes in an 1d element
real(kind=kreal),dimension(:),intent(in) :: xii
real(kind=kreal),intent(in) :: xi ! point where to calculate lagrange function and
!its derivative
real(kind=kreal),dimension(:),intent(out) :: phi,dphi_dxi
...
! Check ubound
if( ubound(xii,1).ne.nenod .or. &
    ubound(phi,1).ne.nenod .or. &
    ubound(dphi_dxi,1).ne.nenod)then
  write(*,*)'ERROR: array sizes do not match!'
  stop
endif
